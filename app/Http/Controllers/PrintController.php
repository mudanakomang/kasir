      ...
      //  $printer -> setJustification(Printer::JUSTIFY_LEFT);
        ...
        foreach($content->produk as $produk){
            if($konter){             
                ...
                if($produk->diskon>0){
                    $harga=$produk->harga;
                    $harga_after_diskon=$harga-($harga*$produk->diskon/100);
                    ...
                    $harga=number_format(($produk->harga*$produk->pivot->jumlah)-($produk->pivot->jumlah*($produk->harga-($produk->harga*$produk->diskon/100))),0,"",".");               
                    $printer->text($this->rows('','Disc ('.$produk->diskon.'%)',"-".$harga));
       ...
       if($konter){         
        ...
        //$printer -> setEmphasis(false);
       // $printer -> setEmphasis(true);
        $printer -> text($this->rows('','Komisi Komisi:', "Rp ".number_format($totalkomisi,0,"","."),true));
       }else{        
